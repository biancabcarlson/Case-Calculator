...
- **Web demo:** pulls in the case's transaction history and lets the investigator
  ...
  record itself, not a per-row choice. Gross Exposure, Confirmed Loss,
  Prevented Loss, and Recovered are shown as currency fields (with $ and
  comma formatting) and auto-filled from the checked transactions, and
  remain manually editable — editing anything after a calculation flags the
  results as stale until you recalculate. A "Copy results" button on the
  results card copies the figures as plain text, ready to paste into a
  report.
