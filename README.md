...
- **Web demo:** pulls in the case's transaction history and lets the investigator
  check any transaction to include it toward the case at all — every row is
  checkable, none are locked out. Which bucket a checked transaction counts
  toward (confirmed loss / prevented loss / recovered) comes from the case
  record itself, not a per-row choice. Pending and reversed transactions carry
  a short caveat next to the checkbox as a heads-up, not a restriction. Gross
  Exposure, Confirmed Loss, Prevented Loss, and Recovered are auto-filled from
  the checked transactions, and remain manually editable — editing anything
  after a calculation flags the results as stale until you recalculate. A
  "Copy results" button on the results card copies the figures as plain text,
