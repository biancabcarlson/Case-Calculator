...
- **Web demo:** pulls in the case's transaction history and lets the investigator
  classify each transaction (Not fraud-related / Confirmed loss / Prevented loss /
  Recovered) from a dropdown. Gross Exposure, Confirmed Loss, Prevented Loss, and
  Recovered are auto-filled from those classifications, and remain manually
  editable — editing anything after a calculation flags the results as stale until
  you recalculate.
