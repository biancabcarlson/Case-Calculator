...
- **CLI:** accepts the same four figures as flags and can emit JSON for another
  tool: `python case_calculator.py --gross 4800 --prevented 4800 --json`.
  With no flags it uses the CASE-DEMO-0091 defaults below.
...
python case_calculator.py --gross 4800 --prevented 4800 --json
```

Python 3, standard library only. Or open `index.html` directly — no server,
framework, or dependencies needed. The browser and CLI share the same
calculation formulas, including a negative remaining exposure when inputs
overlap and need investigator review.
